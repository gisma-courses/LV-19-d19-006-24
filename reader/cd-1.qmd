...
1. Preparing the work environment and automated data collection
2. Overview – Unsupervised classification (k-means clustering)
3. Recording training areas.
4. Supervised classification (Random Forest, Maximum Likelihood)
5. Estimating model quality
...
corine_harz = terra::crop(corine_raw,vect(harz))
plot(corine_harz)
terra::writeRaster(corine_harz,file.path("../data/corine_harz.tif"),overwrite=TRUE)
...
### Aternative 1: Retrieving Sentinel using `gdalcubes`
...
# we "download" the data and write it t a netcdf file
  ...
  raster_cube(s2_collection, v, mask = s2.mask) |>
    write_ncdf("data/harz_2019_summer_tile.nc",overwrite=TRUE)
```
...
## Step2: Overview – Unsupervised classification (k-means clustering)
...
[Digitizing training data in R](digitize2.qmd){.btn .btn-outline-primary .btn role="button" data-toggle="tooltip" title="Digitizing training data using mapedit"}
